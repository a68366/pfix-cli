// Package processes implements the shared `processes` subcommand that lists
// the task or contact processes configured on the account (GET /process/<type>).
package processes
...
// NewCmd builds the `processes` subcommand for the given object type
// ("task" or "contact"). objectType is a fixed internal literal, not user
// input, so it needs no validation and is URL-safe by construction.
func NewCmd(g *cmdutil.GlobalOpts, objectType string) *cobra.Command {
	o := &listOptions{objectType: objectType}
	cmd := &cobra.Command{
		Use:   "processes",
		Short: "List " + objectType + " processes",
		Args:  cobra.NoArgs,
...
func runList(ctx context.Context, o *listOptions) error {
	...
	path := "process/" + o.objectType + "?fields=" + url.QueryEscape(fields)
	client, err := o.client()
	if err != nil {
		return err
	}
	...
	output.Table(o.out, output.ColumnsFor(fields, listDefaultFields, listColumns), env.Processes, !o.quiet)
	return nil
}
