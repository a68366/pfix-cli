...
func TestRunListDefaultTable(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		io.WriteString(w, `{"result":"success","users":[{"id":4,"name":"Alice","lastname":"Smith","email":"alice@example.com","status":"active"},{"id":7,"name":"Bob","lastname":"Jones","email":"bob@example.com","status":"active"}]}`)
	}))
	defer srv.Close()
	...
	if gotPath != "/user/list" {
		t.Errorf("path = %q, want /user/list", gotPath)
	}
	...
	if !strings.Contains(gotBody, `"fields":"id,name,lastname,email,status"`) {
		t.Errorf("body missing default fields: %q", gotBody)
	}
	result := out.String()
	for _, hdr := range []string{"ID", "NAME", "LASTNAME", "EMAIL", "STATUS"} {
		if !strings.Contains(result, hdr) {
			t.Errorf("output missing header %q: %q", hdr, result)
		}
	}
	if !strings.Contains(result, "Alice") {
		t.Errorf("output missing user name Alice: %q", result)
	}
...
func TestRunListJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","users":[]}`)
	}))
	defer srv.Close()
...
func TestRunListQuiet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","users":[{"id":4,"name":"Alice","lastname":"Smith","email":"alice@example.com","status":"active"}]}`)
	}))
	defer srv.Close()
...
func TestRunListCustomFields(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		gotBody = string(b)
		io.WriteString(w, `{"result":"success","users":[{"id":4,"name":"Alice"}]}`)
	}))
	defer srv.Close()
