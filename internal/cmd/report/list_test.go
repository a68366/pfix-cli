...
func TestRunListDefaultTable(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotBody = string(b)
		io.WriteString(w, `{"result":"success","reports":[{"id":1,"name":"Sales"},{"id":2,"name":"Finance"}]}`)
	}))
	defer srv.Close()
	...
	if gotPath != "/report/list" {
		t.Errorf("path = %q, want /report/list", gotPath)
	}
	...
	if !strings.Contains(result, "Sales") {
		t.Errorf("output missing report name Sales: %q", result)
	}
}

func TestRunListJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","reports":[]}`)
	}))
	defer srv.Close()
...
func TestRunListQuiet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","reports":[{"id":1,"name":"Sales"}]}`)
	}))
	defer srv.Close()
