...
func TestRunViewDefaultDetail(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		io.WriteString(w, `{"result":"success","contact":{"id":4,"name":"Alice","midname":"","lastname":"Smith","email":"alice@example.com","phones":[],"isCompany":false,"position":"Engineer","description":"Main contact"}}`)
	}))
	defer srv.Close()
	...
	if !strings.HasPrefix(gotPath, "/contact/4") {
	...
	if !strings.Contains(result, "Alice") {
		t.Errorf("output missing contact name: %q", result)
	}
}

func TestRunViewJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","contact":{"id":4,"name":"Alice","email":"alice@example.com"}}`)
	}))
	defer srv.Close()
...
func TestRunViewCustomFields(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.String()
		io.WriteString(w, `{"result":"success","contact":{"id":4,"name":"Alice"}}`)
	}))
	defer srv.Close()
