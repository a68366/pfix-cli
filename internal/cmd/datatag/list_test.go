...
func TestRunListDefaultTable(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotBody = string(b)
		io.WriteString(w, `{"result":"success","dataTags":[{"id":1,"name":"Alpha"},{"id":2,"name":"Beta"}]}`)
	}))
	defer srv.Close()
	...
	if gotPath != "/datatag/list" {
		t.Errorf("path = %q, want /datatag/list", gotPath)
	}
	...
	if !strings.Contains(result, "Alpha") {
		t.Errorf("output missing datatag name Alpha: %q", result)
	}
}

func TestRunListJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","dataTags":[]}`)
	}))
	defer srv.Close()
...
func TestRunListQuiet(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"result":"success","dataTags":[{"id":1,"name":"Alpha"}]}`)
	}))
	defer srv.Close()
...
func TestRunListCustomFields(t *testing.T) {
	...
	if !strings.Contains(gotBody, `"fields":"id"`) {
